package com.it.one.global;

/**
 * 定义全局参数
 * Created by fenglichao on 17/5/6.
 */

public class GlobalContants {
    //    http://v3.wufazhuce.com:8000/api/onelist/idlist/?channel=wdj&version=4.0.2&uuid=ffffffff-a90e-706a-63f7-ccf973aae5ee&platform=android
    public static final String SERVER_URL = "http://v3.wufazhuce.com:8000/api/onelist/idlist/?channel=wdj&version=4.0.2&uuid=ffffffff-a90e-706a-63f7-ccf973aae5ee&platform=android";
//    http://v3.wufazhuce.com:8000/api/onelist/3528/0?channel=wdj&version=4.0.2&uuid=ffffffff-a90e-706a-63f7-ccf973aae5ee&platform=android
    public static final String PICTURE_SERVER_URl = "http://v3.wufazhuce.com:8000/api/hp/idlist/0?version=3.5.0&platform=android";
    public static final String PICTUER_CONTENT_URL = "http://v3.wufazhuce.com:8000/api/hp/detail/";
    public static final String SERVER_URL_SUFFIX = "?version=3.5.0&platform=android";
}
